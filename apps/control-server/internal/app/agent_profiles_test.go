...
func TestConversationProfileRequestUsesInheritedConfigurationUnlessExplicitlySelected(t *testing.T) {
	...
	var listed []Conversation
	if err := json.NewDecoder(listResponse.Body).Decode(&listed); err != nil {
		t.Fatalf("decode conversation list: %v", err)
	}
	if len(listed) == 0 || listed[0].AgentProfileRevisionID != profile.CurrentRevisionID {
		t.Fatalf("listed profile revision=%q want=%q", listed[0].AgentProfileRevisionID, profile.CurrentRevisionID)
	}
